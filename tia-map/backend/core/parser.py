...
def parse_block_file(xml_path: Path, export_root: Path) -> Block:
    ...
    # Extrai chamadas em LAD/FBD/ST de forma unificada pela tag CallInfo.
    for element in root.iter():
        ...
        instance_element = next(
            (child for child in element if _strip_ns(child.tag) == "Instance"),
            None,
        )
        if instance_element is not None:
            component_element = next(
                (child for child in instance_element if _strip_ns(child.tag) == "Component"),
                None,
            )
            ...
            address_element = next(
                (child for child in instance_element if _strip_ns(child.tag) == "Address"),
                None,
            )
...
def parse_blocks_from_export(export_root: Path) -> list[Block]:
    ...
    for xml_file in xml_files:
        try:
            ...
            # Mantem parser tolerante para arquivos nao-bloco.
            continue
        blocks.append(block)
